...
router.get("/search/:username", async (req, res) => {
  const { username } = req.params;
  db.query(
    "SELECT * FROM users WHERE user_username LIKE ?",
    ["%" + username + "%"],
    (err, result) => {
      if (err) throw err;
      return res.status(200).json(result);
...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    const response = await db
      .promise()
      .query(
        "INSERT INTO users (user_username, user_password, user_pp, user_privilege) VALUES (?, ?, ?, ?)",
        [username, hashedPassword, parseInt(user_pp), "0"]
      );
...
router.post("/update/:id_user", async (req, res) => {
  ...
  try {
    const response = await db
      .promise()
      .query(
        "UPDATE users SET user_username = ?, user_password = ?, user_pp = ? WHERE id_user = ?",
        [username, password, parseInt(avatar), parseInt(id_user)]
      );
    return res.status(200).json(response[0]);
  } catch (err) {
    return res.status(400).json(err);
...
router.post("/update/privilege/:id_user", async (req, res) => {
  ...
  try {
    const response = await db
      .promise()
      .query("UPDATE users SET user_privilege = ? WHERE user_id = ?", [
        parseInt(privilege),
        parseInt(id_user),
      ]);
    return res.status(200).json(response[0]);
...
router.post("/delete/:id_user", async (req, res) => {
  ...
  try {
    const response = await db
      .promise()
      .query("DELETE FROM users WHERE user_id = ?", [parseInt(id_user)]);
    const response1 = await db
      .promise()
      .query("DELETE FROM progress WHERE user_id = ?", [parseInt(id_user)]);
